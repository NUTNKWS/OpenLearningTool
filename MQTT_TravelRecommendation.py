...
def Activate_Hardware(outv_msg):
    ...
    if (outv <= 35): 
        #not recommended
        mySpeaker.playAudio('NotRecommended.mp3')
        myFan.Control_Fan(0)
        myLCD.ShowImage_LCD('travel_notRecommended.bmp', const.LCDImgDelayTime)
        lcdMsg, terminalMsg = organizeMsg(outv_msg, 'Not Recommended.')
        #control LED to flash
        myLED.Flash_ledRED(30, 40)    
           
    elif (outv > 35 and outv <= 65):       
        #recommended
        mySpeaker.playAudio('Recommended.mp3')
        myLCD.ShowImage_LCD('travel_recommended.bmp', const.LCDImgDelayTime)
        lcdMsg, terminalMsg = organizeMsg(outv_msg, 'Recommended.')
        myLED.Flash_ledGREENledRED(30, 40)
                   
    elif (outv > 65):       
        #very recommended
        mySpeaker.playAudio('VeryRecommended.mp3')
        myFan.Control_Fan(700)
        myLCD.ShowImage_LCD('travel_veryRecommended.bmp', const.LCDImgDelayTime)
        lcdMsg, terminalMsg = organizeMsg(outv_msg, 'Very Recommended. GoGo~')
