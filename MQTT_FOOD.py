...
def Activate_Hardware(outv_msg):
    ...
    if (outv <= 2.8):
        #not recommended
        mySpeaker.playAudio('Failure.mp3')
        myFan.Control_Fan(0)
        myLCD.ShowImage_LCD('cry.bmp', const.LCDImgDelayTime)
        lcdMsg, terminalMsg = organizeMsg(outv_msg, 'Not Recommended.')
        #control LED to flash
        myLED.Flash_ledRED(30, 40)    
           
    elif (outv > 2.8 and outv <= 4.8):       
        #recommended
        mySpeaker.playAudio('ding.mp3')
        myLCD.ShowImage_LCD('notbad.bmp', const.LCDImgDelayTime)
        lcdMsg, terminalMsg = organizeMsg(outv_msg, 'Recommended.')
        myLED.Flash_ledGREENledRED(30, 40)
                   
    elif (outv > 4.8):       
        #very recommended
        mySpeaker.playAudio('success.mp3')
        myFan.Control_Fan(700)
        myLCD.ShowImage_LCD('nc.bmp', const.LCDImgDelayTime)
