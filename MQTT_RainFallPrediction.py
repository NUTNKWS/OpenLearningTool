...
def Activate_Hardware(outv_msg):
    ...
    if (outv <= 3.5): 
        #not recommended
        mySpeaker.playAudio('cool.mp3')
        myFan.Control_Fan(700)
        myLCD.ShowImage_LCD('sun.bmp', const.LCDImgDelayTime)
        lcdMsg, terminalMsg = organizeMsg(outv_msg, 'sunny day.')
        #control LED to flash
        myLED.Flash_ledRED(30, 40)    
           
    elif (outv > 3.5 and outv <= 6.5):       
        #recommended
        mySpeaker.playAudio('okok.mp3')
        myLCD.ShowImage_LCD('half.bmp', const.LCDImgDelayTime)
        lcdMsg, terminalMsg = organizeMsg(outv_msg, '50% to rain.')
        myLED.Flash_ledGREENledRED(30, 40)
                   
    elif (outv > 6.5):       
        #very recommended
        mySpeaker.playAudio('sad.mp3')
        myFan.Control_Fan(0)
        myLCD.ShowImage_LCD('rain.bmp', const.LCDImgDelayTime)
        lcdMsg, terminalMsg = organizeMsg(outv_msg, 'rainy day')
