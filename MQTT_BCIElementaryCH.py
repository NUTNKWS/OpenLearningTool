...
def Activate_Hardware(outv_msg):
    ...
    if (outv <= 0.25): 
        #depression
        mySpeaker.playAudio('cs4-BCI-sorrowful.wav')
        myLCD.ShowImage_LCD('cs4-BCI-sorrowful.bmp', const.LCDImgDelayTime)
        lcdMsg, terminalMsg = organizeMsg(outv_msg, 'Sorrowful')
        #control LED to flash
        myLED.Flash_ledGREENledRED(2, 200)       
           
    elif (outv > 0.25 and outv <= 0.45):       
        #anxious
        mySpeaker.playAudio('cs4-BCI-sad.wav')
        myLCD.ShowImage_LCD('cs4-BCI-sad.bmp', const.LCDImgDelayTime)
        lcdMsg, terminalMsg = organizeMsg(outv_msg, 'Sad')
        myLED.Flash_ledRED(2, 200)
                   
    elif (outv > 0.45 and outv <= 0.65):       
        #contentment
        mySpeaker.playAudio('cs4-BCI-happy.wav')
        myLCD.ShowImage_LCD('cs4-BCI-happy.bmp', const.LCDImgDelayTime)
        lcdMsg, terminalMsg = organizeMsg(outv_msg, 'Happy')    
        myLED.Flash_ledGREEN(2, 200)        
           
    else:        
        #exuberance
        mySpeaker.playAudio('cs4-BCI-excited.wav')
        myLCD.ShowImage_LCD('cs4-BCI-excited.bmp', const.LCDImgDelayTime)
        lcdMsg, terminalMsg = organizeMsg(outv_msg, 'Excited')   
