...
def Activate_Hardware(outv_msg):
    ...
    if (outv <= 35): 
        #not recommended
        mySpeaker.playAudio('warning.mp3')
        myFan.Control_Fan(0)
        myLCD.ShowImage_LCD('biggest.bmp', const.LCDImgDelayTime)
        lcdMsg, terminalMsg = organizeMsg(outv_msg, 'You are too close!Watch out!')
        #control LED to flash
        myLED.Flash_ledRED(30, 40)    
           
    elif (outv > 35 and outv <= 65):       
        #recommended
        mySpeaker.playAudio('bibi.mp3')
        ...
        #very recommended
        mySpeaker.playAudio('ifeelgood.mp3')
