...
def Activate_Hardware(outv_msg):
    ...
    if (outv > 7 and outv <= 9.5): 
        #not recommended
        mySpeaker.playAudio('Trick_is_low.wav')
        myFan.Control_Fan(0)
        myLCD.ShowImage_LCD('Trick_is_low.bmp', const.LCDImgDelayTime)
        lcdMsg, terminalMsg = organizeMsg(outv_msg, 'Trick is low.')
        #control LED to flash
        myLED.Flash_ledRED(30, 40)    
           
    elif (outv > 9.5 and outv <= 11.5):       
        #recommended
        mySpeaker.playAudio('Trick_is_medium.wav')
        myLCD.ShowImage_LCD('Trick_is_medium.bmp', const.LCDImgDelayTime)
        lcdMsg, terminalMsg = organizeMsg(outv_msg, 'Trick is medium.')
        myLED.Flash_ledGREENledRED(30, 40)
                   
    elif (outv > 11.5):       
        #very recommended
        mySpeaker.playAudio('Trick_is_high.wav')
        myFan.Control_Fan(700)
        myLCD.ShowImage_LCD('Trick_is_high.bmp', const.LCDImgDelayTime)
        lcdMsg, terminalMsg = organizeMsg(outv_msg, 'Trick is high.')
