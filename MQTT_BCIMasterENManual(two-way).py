...
#inferred result: 0.127, 0.4, 0.759, 0.922, and 0.7
input_data = [['2.3', '5.5', '4.5', '5.1', '4.2'],
              ['4.2', '3.8', '6.2', '6.1', '7'],
              ['7.3', '5.1', '4.9', '3.5', '3.8'],
              ['9.4', '4.9', '5.1', '1.3', '2.1'],
              ['7', '5', '5', '6', '9']]
...
class AILearningPlatform:
    ...
    def Activate_Hardware(self, outv_msg):
        ...
        if (outv <= 0.25): 
            #depression
            self.mySpeaker.playAudio('cs4-BCI-Depression.wav')
            self.myLCD.ShowImage_LCD('cs4-BCI-Depression.bmp', const.LCDImgDelayTime)
            lcdMsg, terminalMsg = self.organizeMsg(outv_msg, 'Depression')
            #control LED to flash
            self.myLED.Flash_ledGREENledRED(2, 200)       
               
        elif (outv > 0.25 and outv <= 0.45):       
            #anxious
            self.mySpeaker.playAudio('cs4-BCI-Anxious.wav')
            self.myLCD.ShowImage_LCD('cs4-BCI-Anxious.bmp', const.LCDImgDelayTime)
            lcdMsg, terminalMsg = self.organizeMsg(outv_msg, 'Anxious')
            self.myLED.Flash_ledRED(2, 200)
                       
        elif (outv > 0.45 and outv <= 0.85):       
            #contentment
            self.mySpeaker.playAudio('cs4-BCI-Contentment.wav')
            self.myLCD.ShowImage_LCD('cs4-BCI-Contentment.bmp', const.LCDImgDelayTime)
            lcdMsg, terminalMsg = self.organizeMsg(outv_msg, 'Contentment')    
            self.myLED.Flash_ledGREEN(2, 200)        
               
        else:        
            #exuberance
            self.mySpeaker.playAudio('cs4-BCI-Exuberance.wav')
            self.myLCD.ShowImage_LCD('cs4-BCI-Exuberance.bmp', const.LCDImgDelayTime)
            lcdMsg, terminalMsg = self.organizeMsg(outv_msg, 'Exuberance')   
            self.myLED.Flash_ledGREENledREDONOFF(2, 200)
