...
def Activate_Hardware(outv_msg):
    ...
    if (outv < 3.5): 
        #not recommended
        mySpeaker.playAudio('PowerEstimation_Low.mp3')
        myFan.Control_Fan(0)
        myLCD.ShowImage_LCD('PowerEstimation_Low.bmp', const.LCDImgDelayTime)
        lcdMsg, terminalMsg = organizeMsg(outv_msg, 'Low...')
        #control LED to flash
        myLED.Flash_ledRED(30, 40)    
           
    elif (outv >= 3.5 and outv < 6.5):       
        #recommended
        mySpeaker.playAudio('PowerEstimation_Medium.mp3')
        myLCD.ShowImage_LCD('PowerEstimation_Medium.bmp', const.LCDImgDelayTime)
        lcdMsg, terminalMsg = organizeMsg(outv_msg, 'Medium')
        myLED.Flash_ledGREENledRED(30, 40)
                   
    elif (outv >= 6.5):       
        #very recommended
        mySpeaker.playAudio('PowerEstimation_High.mp3')
        myFan.Control_Fan(700)
        myLCD.ShowImage_LCD('PowerEstimation_High.bmp', const.LCDImgDelayTime)
        lcdMsg, terminalMsg = organizeMsg(outv_msg, 'HIGH!!!')
